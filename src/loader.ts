 ...
 */
export function resolveSearchPaths(): string[] {
  const paths: string[] = [];
  ...
  if (workspaceRootsEnv) {
    for (const root of workspaceRootsEnv.split(path.delimiter)) {
      ...
      if (fs.existsSync(pbPath)) paths.push(pbPath);
    }
  }
  ...
  if (fs.existsSync(projectPlaybooks)) paths.push(projectPlaybooks);
 ...
 */
export function discoverPlaybooks(): PlaybookDefinition[] {
 ...
 */
export function loadPlaybook(name: string): PlaybookDefinition | null {
  const searchPaths = resolveSearchPaths();
  for (const sp of searchPaths) {
 ...
 */
export function parsePlaybookFile(filePath: string): PlaybookDefinition {
...
function normalizeSteps(raw: unknown): PlaybookStep[] | undefined {
...
function topologicalSort(steps: PlaybookStep[]): PlaybookStep[] {
...
function resolveAllDependencies(
  ...
  allPlaybooks: PlaybookDefinition[],
): Map<string, string[]> {
